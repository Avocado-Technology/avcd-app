#!/usr/bin/env bash
# Upload secret keys from a local dotenv file into Infisical project avcd-web.
# Only keys listed in config/infisical-secret-keys.list are uploaded.
# Public variables belong in config/deploy.yml (Kamal env.clear), not Infisical.
set -euo pipefail
...
INFISICAL_PROJECT_ID="${INFISICAL_PROJECT_ID:-4c32b3c4-fb30-44a2-81bb-2ae4211404a3}"
INFISICAL_SECRET_PATH="${INFISICAL_SECRET_PATH:-/}"
INFISICAL_ENV="${INFISICAL_ENV:-dev}"
INFISICAL_PUSH_FILE="${INFISICAL_PUSH_FILE:-.env.local}"
...
if [[ ! -f "$INFISICAL_PUSH_FILE" ]]; then
  ...
  echo "   Copy .env.local.example → .env.local and add secrets, or set INFISICAL_PUSH_FILE." >&2
  exit 1
fi
...
if [[ "$count" -eq 0 ]]; then
  ...
  echo "   Skipped $skipped non-secret key(s). Put public config in config/deploy.yml (env.clear)." >&2
  exit 1
fi
...
echo "✓ Uploaded $count secret(s) from $INFISICAL_PUSH_FILE → avcd-web ($INFISICAL_ENV @ $INFISICAL_SECRET_PATH)"
